  ...
  loadCompanies(): void {
    this.companyService
      ...
      .subscribe((response: any) => {
        this.companies.data = response.content; // Данни за текущата страница
        this.totalCompanies = response.totalElements; // Общо компании
      });
  }

  onSearch(event: Event): void {
    const input = event.target as HTMLInputElement;
    this.searchSubject.next(input.value); // Изпращане на стойността към `searchSubject`
